...
const getSubmissionInfo = (s, displayConfig) => ({
    ...
    language: displayConfig.showCode
        ? s.language != null && s.language !== ''
            ? syzoj.languages[s.language].show
            : null
        : null,
...
const getRoughResult = (x, displayConfig, roughOnly) => {
    if (displayConfig.showResult) {
        ...
        // 0: Waiting 1: Running
        if (x.status === 'System Error') return { result: 'System Error' };
        if (x.compilation == null || [0, 1].includes(x.compilation.status)) {
            return null;
        } else {
            if (x.compilation.status === 2) {
                // 2 is TaskStatus.Done
...
const processOverallResult = (source, config) => {
    ...
    return {
        ...
        judge: config.showDetailResult
            ? source.judge && {
                  subtasks:
                      source.judge.subtasks &&
                      source.judge.subtasks.map((st) => ({
                          score: st.score,
                          cases: st.cases.map((cs) => ({
                              ...
                              result: cs.result && {
                                  type: cs.result.type,
                                  time: config.showUsage
                                      ? cs.result.time
                                      : undefined,
                                  memory: config.showUsage
                                      ? cs.result.memory
                                      : undefined,
                                  scoringRate: cs.result.scoringRate,
                                  systemMessage: cs.result.systemMessage,
                                  input: config.showTestdata
                                      ? cs.result.input
                                      : undefined,
                                  output: config.showTestdata
                                      ? cs.result.output
                                      : undefined,
                                  userOutput: config.showTestdata
                                      ? cs.result.userOutput
                                      : undefined,
                                  userError: config.showTestdata
                                      ? cs.result.userError
                                      : undefined,
