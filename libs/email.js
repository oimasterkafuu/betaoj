...
if (syzoj.config.email.method === 'sendmail') {
    ...
    const smtpConfig = {
        ...
        secure:
            syzoj.config.email.options.port === 465 ||
            !syzoj.config.email.options.port
                ? true
                : false,
        auth: {
    ...
    doSendEmail = async function send_smtp(to, subject, body) {
        await transporter.sendMailAsync({
            from: `"${syzoj.config.title}" <${
                syzoj.config.email.options.address ||
                syzoj.config.email.options.username
            }>`,
            to: to,
            subject: subject,
            html: body,
