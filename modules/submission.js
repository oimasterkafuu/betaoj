...
app.get('/submissions', async (req, res) => {
    try {
        ...
        if (!req.query.contest) {
            ...
            if (
                (contest.ended && contest.is_public) || // If the contest is ended and is not hidden
        ...
        if (
            !inContest &&
            (!curUser || !(await curUser.hasPrivilege('manage_problem')))
        ) {
...
app.get('/submission/:id', async (req, res) => {
    try {
        ...
        if (judge.type === 1) {
            ...
            if (
                (!contest.ended || !contest.is_public) &&
                !(
                    (await judge.problem.isAllowedEditBy(res.locals.user)) ||
        ...
        res.render('submission', {
            ...
            code:
                judge.problem.type !== 'submit-answer'
            ...
            socketToken:
                judge.pending && judge.task_id != null
                    ? jwt.sign(
                          {
...
app.post('/submission/:id/rejudge', async (req, res) => {
    try {
        ...
        if (
            ...
            throw new ErrorMessage('无法重新评测一个评测中的提交。');
...
app.post('/submission/:id/skip', async (req, res) => {
    try {
        ...
        if (
            ...
            throw new ErrorMessage('无法跳过一个评测中的提交。');
