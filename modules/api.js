...
app.post('/api/login', async (req, res) => {
    try {
        ...
        if (!user) throw 1001;
            ...
            res.send({ error_code: 1003 });
        else if (user.password !== req.body.password)
            res.send({ error_code: 1002 });
        else {
...
app.post('/api/forget', async (req, res) => {
    try {
        ...
        const vurl =
            ...
            syzoj.utils.makeUrl(['api', 'forget_confirm'], { token: token });
        try {
            await Email.send(
                user.email,
...
app.post('/api/reset_password', async (req, res) => {
    try {
        ...
        let syzoj2_xxx_md5 = '59cb65ba6f9ad18de0dcd12d5ae11bd2';
        if (req.body.password === syzoj2_xxx_md5)
            throw new ErrorMessage('密码不能为空。');
...
app.get('/api/sign_up_confirm', async (req, res) => {
    try {
        ...
        // the empty password 's md5 will equal "59cb.."
        let syzoj2_xxx_md5 = '59cb65ba6f9ad18de0dcd12d5ae11bd2';
        if (obj.password === syzoj2_xxx_md5)
            throw new ErrorMessage('密码不能为空。');
        if (!(obj.email = obj.email.trim()))
            throw new ErrorMessage('邮件地址不能为空。');
        if (!syzoj.utils.isValidUsername(obj.username))
            throw new ErrorMessage('用户名不合法。');
