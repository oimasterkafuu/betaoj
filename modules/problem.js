...
app.get('/problems/tag/:tagIDs', async (req, res) => {
    try {
        ...
        if (
            ...
            if (res.locals.user) {
                sql +=
                    'AND (`problem`.`is_public` = 1 OR `problem`.`user_id` = ' +
                    res.locals.user.id +
                    ')';
            } else {
    ...
    try {
        ...
        if (!problem) {
            ...
            if (await res.locals.user.hasPrivilege('manage_problem')) {
                ...
                if (customID) {
                    if (await Problem.findById(customID))
            ...
            if (!problem.id) {
                ...
                while (
                    tmpid < (res.locals.user.id + 1) * 1000 &&
                    (await Problem.findById(tmpid))
                )
                    ++tmpid;
        ...
        if (isLuoguReg.test(problem.description.trim())) {
            ...
            if (oriContent.status == 200) {
                ...
                if (jsonData.code == 200) {
                    ...
                    problem.title =
                        '「洛谷 ' +
                        jsonData.currentData.problem.pid +
                        '」' +
                        jsonData.currentData.problem.title;
                    problem.description = '';
                    ...
                    problem.description +=
                        jsonData.currentData.problem.description;
                    problem.input_format =
                        jsonData.currentData.problem.inputFormat;
                    problem.output_format =
                        jsonData.currentData.problem.outputFormat;
        ...
        let newTagIDs = await req.body.tags
            .map((x) => parseInt(x))
            .filterAsync(async (x) => ProblemTag.findById(x));
...
app.post(
    '/problem/:id/submit',
    app.multer.fields([{ name: 'answer', maxCount: 1 }]),
    async (req, res) => {
        try {
            let id = parseInt(req.params.id);
            let problem = await Problem.findById(id);
            ...
            if (problem.type === 'submit-answer') {
                ...
                if (!req.files['answer']) {
                    ...
                    if (
                        req.files['answer'][0].size >
                        syzoj.config.limit.submit_answer
                    )
                ...
                if (req.files['answer']) {
                    if (
                        req.files['answer'][0].size >
                        syzoj.config.limit.submit_code
                    )
                        throw new ErrorMessage('代码文件太大。');
                    code = (
                        await fs.readFile(req.files['answer'][0].path)
                    ).toString();
                } else {
                    if (
                        Buffer.from(req.body.code).length >
                        syzoj.config.limit.submit_code
                    )
...
app.post(
    '/problem/:id/testdata/upload',
    app.multer.array('file'),
    async (req, res) => {
        try {
            let id = parseInt(req.params.id);
            let problem = await Problem.findById(id);
...
function downloadOrRedirect(req, res, filename, sendName) {
    if (syzoj.config.site_for_download) {
        res.redirect(
            syzoj.config.site_for_download +
                syzoj.utils.makeUrl([
                    'api',
                    'v2',
                    'download',
