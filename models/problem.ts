...
export default class Problem extends Model {
    ...
    async uploadTestdataSingleFile(filename, filepath, size, noLimit) {
        await syzoj.utils.lock(['Promise::Testdata', this.id], async () => {
            ...
            if (
                !noLimit &&
                oldCount + (!replace as any as number) >
                    syzoj.config.limit.testdata_filecount
            )
                throw new ErrorMessage('数据包中的文件太多。');
    ...
    async makeTestdataZip() {
        await syzoj.utils.lock(['Promise::Testdata', this.id], async () => {
            let dir = this.getTestdataPath();
            if (!(await syzoj.utils.isDir(dir)))
    ...
    async hasSpecialJudge() {
        try {
            ...
            return (
                list.includes('spj.js') ||
                list.find((x) => x.startsWith('spj_')) !== undefined
            );
        } catch (e) {
            return false;
        }
    ...
    async validate() {
        ...
        if (this.time_limit > syzoj.config.limit.time_limit)
            return 'Time limit too large';
        if (this.memory_limit <= 0) return 'Invalid memory limit';
        if (this.memory_limit > syzoj.config.limit.memory_limit)
            return 'Memory limit too large';
        if (
            !['traditional', 'submit-answer', 'interaction'].includes(this.type)
        )
        ...
        if (this.type === 'traditional') {
            let filenameRE = /^[\w \-\+\.]*$/;
            if (
                this.file_io_input_name &&
                !filenameRE.test(this.file_io_input_name)
            )
                return 'Invalid input file name';
            if (
                this.file_io_output_name &&
                !filenameRE.test(this.file_io_output_name)
            )
    ...
    async resetSubmissionCount() {
        await syzoj.utils.lock(
            ['Problem::resetSubmissionCount', this.id],
            async () => {
    ...
    async updateStatistics(user_id) {
        await Promise.all(
            Object.keys(statisticsTypes).map(async (type) => {
                if (
                    this.type === ProblemType.SubmitAnswer &&
                    statisticsCodeOnly.includes(type)
                )
                    return;

                await syzoj.utils.lock(
                    ['Problem::UpdateStatistics', this.id, type],
                    async () => {
    ...
    async getStatistics(type, paginate) {
        ...
        const ids = (
            await SubmissionStatistics.queryPage(
                paginate,
                {
                    problem_id: this.id,
                    type: type
        ...
        statistics.judge_state = ids.length
            ? await JudgeState.createQueryBuilder()
                  .whereInIds(ids)
    ...
    async changeID(id) {
        ...
        let oldTestdataDir = this.getTestdataPath(),
            oldTestdataZip = this.getTestdataArchivePath();

        const oldID = this.id;
        this.id = id;

        // Move testdata
        let newTestdataDir = this.getTestdataPath(),
            newTestdataZip = this.getTestdataArchivePath();
    ...
    async delete() {
        ...
        let oldTestdataDir = this.getTestdataPath(),
            oldTestdataZip = this.getTestdataPath();
