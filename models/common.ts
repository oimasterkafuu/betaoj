...
export default class Model extends TypeORM.BaseEntity {
    ...
    static async countForPagination(where) {
        const queryBuilder =
            where instanceof TypeORM.SelectQueryBuilder
                ? where
    ...
    static async queryPage(
        ...
        queryBuilder
            .skip((paginater.currPage - 1) * paginater.perPage)
    ...
    static async queryPageFast<T extends TypeORM.BaseEntity>(
        this: TypeORM.ObjectType<T>,
        ...
        await Promise.all(
            [
                async () =>
                    (result.meta.hasPrevPage = !!(await queryBuilderHasPrev
                        .andWhere(
                            `id ${
                                idOrder === PaginationIDOrder.DESC ? '>' : '<'
                            } :id`,
                        ...
                        .getOne())),
                async () =>
                    (result.meta.hasNextPage = !!(await queryBuilderHasNext
                        .andWhere(
                            `id ${
                                idOrder === PaginationIDOrder.DESC ? '<' : '>'
                            } :id`,
    ...
    static async queryRange(range: any[], where, order) {
        ...
        queryBuilder.skip(range[0] - 1).take(range[1] - range[0] + 1);
