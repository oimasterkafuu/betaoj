...
export default class Contest extends Model {
    ...
    allowedSeeingScore() {
        // If not, then the user can only see status
        if (this.type === 'ioi') return true;
        else return false;
    }

    allowedSeeingResult() {
        // If not, then the user can only see compile progress
    ...
    async newSubmission(judge_state) {
        if (
            !(
                judge_state.submit_time >= this.start_time &&
                judge_state.submit_time <= this.end_time
            )
        ) {
            return;
        }
        ...
        await syzoj.utils.lock(
            ['Contest::newSubmission', judge_state.user_id],
            async () => {
