...
module.exports = {
    ...
    markdown(obj, keys, noReplaceUI) {
        let replaceUI = (s) => {
            ...
            s = s
                ...
                .join('</pre></div>')
                .split('<table>')
                .join('<table class="ui structured celled table">')
                .split('<blockquote>')
                .join('<div class="ui message">')
                .split('</blockquote>')
                .join('</div>');
    ...
    gravatar(email, size) {
        return gravatar
            .url(email, { s: size, d: 'mm' })
            .replace('//www.gravatar.com/avatar', syzoj.config.gravatar_url);
    },
    async parseTestdata(dir, submitAnswer) {
        ...
        try {
            ...
            if (!list.includes('data.yml')) {
                ...
                res[0].type = 'sum';
                res[0].score = 100;
                res[0].cases.forEach((e) => {
                    e.key = (e.input.match(/\d+/g) || [])
                        .map((x) => parseInt(x))
                        .concat(e.input);
                });
                res[0].cases.sort((a, b) => {
                    for (
                        let i = 0;
                        i < Math.max(a.key.length, b.key.length);
                        ++i
                    ) {
